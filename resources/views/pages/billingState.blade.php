...
<div class="container-fluid p-3">
    <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col">
                    <span class="schedule-label">BILLING STATEMENT</span>
                    <p class="module-description">Use this module to view billing statement...</p>
                    ...
                    <div class="col d-flex flex-row-reverse">
                            ...
                            <button type="button" class="btn-sm btn btn-outline-primary addBtn w-full"><i
                                        class="fa-solid fa-plus mr-1"></i>New Billing</button>
                            </section>
                        </div>
                    </div>
                </div>
                                ...
                                <tr>
                                    <td>
                                        <center class="mt-3"><input class="form-check-input checkbox" type="checkbox"
                                                value="{{ $fetchData->billing_id }}"></center>
                                    </td>
                                    <td>
                                        <p class="mt-3 text-center">{{ $fetchData->billing_name }}</p>
                                    </td>
                                    <td class="text-center">
                                        <p class="mt-3"><a href="{{ route('downloadBill', $fetchData->billing_file) }}"
                                                class="btn-sm previewBtn w-full"><i
                                                    class="fa-solid fa-download mr-2"></i>Download</a></p>
                                    </td>
                                    <td>
                                        <p class="mt-3 text-center">{{ date('Y-m-d', strtotime($fetchData->created_at)) }}</p>
                                    </td>
                                    <td>
                                        <center class="mt-3">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li class="p-2">
                                                        <button data-id="{{ $fetchData->billing_id }}"
        ...
        <script>
            ...
            $(document).on('submit', '#submitNewUser', async function(e) {
                ...
                let response = await fetch("{{ route('manageBilling') }}", {
                    method: 'POST',
            ...
            $(document).on('click', '.editBtn', function(e) {
                const showUser = async (id) => {
                    let response = await fetch(`showBilling/${id}`, {
                        method: 'GET',
                        credentials: "same-origin",
                    });
                    ...
                    if (status == 'success') {
                        message.map(el => {
                            $('#id').val(el.billing_id)
                            $('#billingname').val(el.billing_name)
            ...
            $(document).on('click', '.removeBtn', async function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        let response = await fetch("{{ route('destroyBilling') }}", {
                            method: 'POST',
                            credentials: "same-origin",
                            headers: {
