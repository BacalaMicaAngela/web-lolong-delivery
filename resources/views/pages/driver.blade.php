@extends('pages.navigator')
@section('contentAdmin')
<style>
    .schedule-label {
        font-size: 20px; /* Adjust the size as needed */
        font-weight: bold; /* Make the text bold */
    }

    .module-description {
        font-size: 12px; /* Adjust the size as needed */
        color: #6B7280; /* Set the color to a muted tone */
    }
</style>

<div class="container-fluid p-3">
    <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col">
                    <span class="schedule-label">DRIVER LIST</span>
                    <p class="module-description">Use this module to view your drivers...</p>
                </div>
                    <div class="col d-flex flex-row-reverse">
                            <section class="p-2">
                            <button type="button" class="btn-sm btn btn-outline-danger w-full removeBtn"><i
                                        class="fa-solid fa-trash mr-1"></i>Remove From List</button>
                            </section>
                            <div class="dropdown-divider"></div>
                            <section class="p-2">
                            <button type="button" class="btn-sm btn btn-outline-primary addBtn w-full"><i
                                        class="fa-solid fa-plus mr-1"></i>New Driver</button>
                            </section>
                        </div>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <div style="overflow: auto;">
                    <table id="dataTable" class="table table-bordered" style="width: 100%;">
                        <thead>
                            <tr>
                                <th>
                                    <center>
                                        <input class="form-check-input" type="checkbox" id="selectAll">
                                        <label class="form-check-label">
                                            Select All
                                        </label>
                                    </center>
                                </th>
                                <th class="text-center">Driver Name</th>
                                <th class="text-center">Age</th>
                                <th class="text-center">Address</th>
                                <th class="text-center">Mobile No.</th>
                                <th class="text-center">License No.</th>
                                <th class="text-center">Status</th>
                                <th class="text-center">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($fetch as $fetchData)
                                <tr>
                                    <td>
                                        <center class="mt-3"><input class="form-check-input checkbox" type="checkbox"
                                                value="{{ $fetchData->driver_id }}"></center>
                                    </td>
                                    
                                    <td>
                                        <p class="mt-3">{{ $fetchData->driver_name }}</p>
                                    </td>
                                    <td>
                                        <p class="mt-3">{{ $fetchData->age }}</p>
                                    </td>
                                    <td>
                                        <p class="mt-3">{{ $fetchData->driver_address }}</p>
                                    </td>
                                    <td>
                                        <p class="mt-3">{{ $fetchData->driver_phone }}</p>
                                    </td>
                                    <td>
                                        <p class="mt-3">{{ $fetchData->license_no }}</p>
                                    </td>
                                <td>
                                        <center class="mt-3">
                                            @if ($fetchData->status==0)
                                            <span class="badge bg-success">Available</span>
                                            @else
                                                <span class="badge bg-danger">Unavailable</span>
                                            @endif
                                        </center>
                                    </td>
                                    <td>
                                        <center class="mt-3">
                                            @if ($fetchData->status==1)
                                                <button class="btn-sm btn btn-outline-primary restoreBtn w-full" data-id="{{ $fetchData->driver_id }}"> 
                                                    <i class="fa-solid fa-recycle mr-2"></i>Restore</button>
                                            @else
                                                <div class="dropdown">
                                                    <button class="btn-sm btn btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                    Action
                                                    </button>
                                                    <ul class="dropdown-menu">
                                                        <li class="p-2">
                                                            <button data-id="{{ $fetchData->driver_id }}" class="btn-sm btn btn-outline-success editBtn w-full">
                                                                <i class="fa-solid fa-pen mr-2"></i>Edit</button>
                                                        </li>
                                                        <li class="p-2">
                                                            <button class="btn-sm btn btn-outline-danger removeBtnCheck w-full" data-id="{{ $fetchData->driver_id }}"> 
                                                                <i class="fa-solid fa-trash mr-2"></i>Switch Status</button>
                                                        </li>
                                                    </ul>
                                                </div>
                                            @endif
                                        </center>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
        @include('pages.Modals.driverForm')
        <script>
            function Component() {
                this.msg = (msg, icon) => {
                    Swal.fire({
                        position: 'center',
                        icon: icon,
                        text: msg,
                        showConfirmButton: false,
                        timer: 2000
                    });
                }

                this.checkInput = () => {
                    if ($('#helper').val() == '' || $('#truckercode').val() == '') {
                        return false;
                    } else {
                        return true;
                    }
                }
                this.removeDriver = async (id) => {
            let response = await fetch(`removeDriver/${id}`, {
                method: 'POST',
                credentials: "same-origin",
                headers: {
                    "X-CSRF-Token": document.querySelector('input[name=_token]').value
                },
            });

            let { message, status } = await response.json();

            if (status == 'success') {
                this.msg(
                    message,
                    status
                );
                setTimeout(() => {
                    location.reload();
                }, 2000);
            } else {
                this.msg(
                    message,
                    'error'
                );
            }
        }

        this.restoreDriver = async (id) => {
            let response = await fetch(`restoreDriver/${id}`, {
                method: 'POST',
                credentials: "same-origin",
                headers: {
                    "X-CSRF-Token": document.querySelector('input[name=_token]').value
                },
            });

            let { message, status } = await response.json();

            if (status == 'success') {
                this.msg(
                    message,
                    status
                );
                setTimeout(() => {
                    location.reload();
                }, 2000);
            } else {
                this.msg(
                    message,
                    'error'
                );
            }
        }
    
            }

            $(document).on('click', '.addBtn', function(e) {
                $('#submitNewUser')[0].reset();
                $('#manageUserLabel').html('New Data Form');
                $('#manageUser').modal('show');
            });

            $(document).on('submit', '#submitNewUser', async function(e) {
                e.preventDefault();

                let comp = new Component();

                if (!comp.checkInput()) {
                    comp.msg(
                        'Warning: Fill up all the required fields.',
                        'warning'
                    );
                    return;
                }

                let response = await fetch("{{ route('manageDriver') }}", {
                    method: 'POST',
                    credentials: "same-origin",
                    headers: {
                        "X-CSRF-Token": document.querySelector('input[name=_token]').value
                    },
                    body: new FormData(document.querySelector('#submitNewUser'))
                });

                let {
                    message,
                    status
                } = await response.json();

                if (status == 'success') {
                    comp.msg(
                        message,
                        status
                    );
                    $('#manageUser').modal('hide');
                    setTimeout(() => {
                        location.reload();
                    }, 2000);

                }  else if (status == 'warning') {
                    comp.msg(
                        message,
                        status
                    );
                } else {
                    console.error(message);
                }
            });

            $('#selectAll').click(function(event) {
                if (this.checked) {
                    $('.checkbox').each(function() {
                        $(this).prop('checked', true);
                    });
                } else {
                    $('.checkbox').each(function() {
                        $(this).prop('checked', false);
                    });
                }
            });

            $(document).on('click', '.editBtn', function(e) {
                const showUser = async (id) => {
                    let response = await fetch(`showDriver/${id}`, {
                        method: 'GET',
                        credentials: "same-origin",
                    });

                    let {
                        message,
                        status
                    } = await response.json();

                    if (status == 'success') {
                        message.map(el => {
                            $('#name').val(el.driver_name )
                            $('#age').val(el.age)
                            $('#mobile').val(el.driver_phone)
                            $('#address').val(el.driver_address)
                            $('#licenseno').val(el.license_no)
                            $('#driver_id').val(el.driver_id )
                            
                            $('#manageUser').modal('show');
                            $('#manageUserLabel').html('Update Data Form');
                        });


                    } else {
                        console.error(message);
                    }
                }
                showUser($(this).data('id'));
            });

            $(document).on('click', '.removeBtn', async function(e) {
                e.preventDefault();
                Swal.fire({
                    text: 'Are you sure?',
                    text: "Do you want to remove",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Remove'
                }).then(async (result) => {
                    if (result.isConfirmed) {
                        let comp = new Component();
                        let data = $('input:checkbox:checked');
                        let formData = new FormData();
                        data.map((i, el) =>
                            formData.append(i, el.value)
                        );
                        let response = await fetch("{{ route('destroyDriver') }}", {
                            method: 'POST',
                            credentials: "same-origin",
                            headers: {
                                "X-CSRF-Token": document.querySelector('input[name=_token]')
                                    .value
                            },
                            body: formData
                        });

                        let {
                            message,
                            status
                        } = await response.json();

                        if (status == 'success') {
                            comp.msg(
                                message,
                                status
                            );
                            setTimeout(() => {
                                location.reload();
                            }, 2000);
                        } else if (status == 'warning') {
                            comp.msg(
                                message,
                                status
                            );
                        } else {
                            console.error(message);
                        }
                    }
                });
            });

            $(document).on('click', '.removeBtnCheck', async function(e) {
                let id=$(this).data('id');
                approveType(
                    id,
                    '1'
                );
            });

            $(document).on('click', '.restoreBtn', async function(e) {
                let id=$(this).data('id');
                approveType(
                    id,
                    '0'
                );
            });

            const approveType = async (id, type) => {
                let msg = type == '0' ? 'active' : 'inactive';

                Swal.fire({
                    title: 'Are you sure?',
                    text: `Switch Driver Status to ${msg}`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Proceed'
                }).then( async (result) => {
                    if (result.isConfirmed) {
                        let comp = new Component();
                        let response = await fetch(`actionDriver/${id}/${type}`, {
                            // Ensure it's a POST request
                            credentials: 'same-origin'
                        });

                        let { message, status } = await response.json();
                        console.log(message,status)

                        if(status == 'success') {
                            comp.msg(
                                message,
                                status
                            );
                            setTimeout(() => {
                                location.reload();
                            }, 2000);
                        } else {
                            comp.msg(
                                message,
                                'error'
                            );
                        }
                    }
                })
            }
        </script>
    </div>
@endsection
@include('pages.Modals.manageProfile')
